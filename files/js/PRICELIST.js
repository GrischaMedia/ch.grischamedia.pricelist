if (!PRICELIST) var PRICELIST = {};

/**
 * Copy of WCF.Label.Chooser by WoltLab with minor changes; taken from UZ Community Bot
 * 
 */
/**
 * Provides a flexible label chooser.
 */
PRICELIST.PricelistLabelChooser = Class.extend({
	/**
	 * label container
	 */
	_container: null,
	...
	_initContainers: function(containerSelector) {
		$(containerSelector).find('.pricelistLabelChooser').each($.proxy(function(index, group) {
			var $group = $(group);
	...
	_submit: function() {
		...
		for (var $groupID in this._groups) {
			...
			if ($group.data('labelID')) {
				$('<input type="hidden" name="pricelistLabelIDs[' + $groupID + ']" value="' + $group.data('labelID') + '" />').appendTo($formSubmit);
			}
		}
	}
});
